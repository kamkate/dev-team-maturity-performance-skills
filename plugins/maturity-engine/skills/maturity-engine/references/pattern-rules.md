...
> the single source of truth for pattern trigger rules (and the
> `pattern_aliases` remap from OS2a) is `maturity-core/engine_defaults.json`
> (`patterns` and `pattern_aliases` blocks), implemented in
> `run_analysis.py::detect_patterns`. This file is kept for historical and
> thesis reference only — it documents business-impact narrative per pattern
> that engine_defaults.json does not carry. Do not edit this file expecting
> it to change engine behavior.

This file previously defined how KPI levels map to diagnostic patterns,
retained below for narrative/thesis context. Patterns are the bridge
between measured KPI values and catalog interventions. All pattern
detection is deterministic — same KPI levels always produce same patterns.
No LLM judgment in pattern detection.
